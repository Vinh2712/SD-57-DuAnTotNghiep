package com.example.sd_57_datn.repository.ChuongTrinhGiamGia;
...
//    List<ChuongTrinhGiamGiaHoaDon> findByKeywordAndDate(String keyword, String min, String max);
    ...
    @Query(value = "SELECT hd.* FROM ChuongTrinhGiamGiaHoaDon hd WHERE hd.soLuongSanPham <= :sl AND hd.soTienHoaDon <= :tt AND hd.trangThai != -1 AND hd.ngayBatDau <= CAST(GETDATE() AS DATE)",nativeQuery = true)
